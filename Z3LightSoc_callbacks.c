...
#define LIGHT_ENDPOINT1 (1)
#define LIGHT_ENDPOINT2 (2)
...
void
commissioningLedEventHandler (void)
{
  ...
  if (emberAfNetworkState () == EMBER_JOINED_NETWORK)
    {
      if (lastButton == BUTTON0) {
          ...
          emberAfCorePrintln("%p network %p: 0x%X", "Leave", "start", status);
      }
    }
  else
  {
...
{
  emberAfCorePrintln("%p network %p: 0x%X", "Form distributed", "complete",
                     EMBER_SUCCESS);
}
...
emberAfOnOffClusterServerAttributeChangedCallback (
    uint8_t endpoint, EmberAfAttributeId attributeId)
{
          ...
          if (onOff)
            {
              switch (endpoint)
                {
                case LIGHT_ENDPOINT1:
                  halSetLed (RELAY1_OFF_LED);
                  halClearLed (RELAY1);
                  break;
                case LIGHT_ENDPOINT2:
                  halSetLed (RELAY2_OFF_LED);
                  halClearLed (RELAY2);
                  break;
                }
            }
          else
            {
              switch (endpoint)
                ...
                case LIGHT_ENDPOINT1:
                  halSetLed (RELAY1);
                  halClearLed (RELAY1_OFF_LED);
                  break;
                case LIGHT_ENDPOINT2:
                  halSetLed (RELAY2);
                  halClearLed (RELAY2_OFF_LED);
                  break;
                }
            }
        }
 ...
 */
void
emberAfHalButtonIsrCallback (uint8_t button, uint8_t state)
{
  lastButton = button;
  ...
  if (state == BUTTON_RELEASED)
    {
      ...
      switch (button)
        {
        case BUTTON0:
          endpoint = LIGHT_ENDPOINT1;
          break;
        case BUTTON1:
          endpoint = LIGHT_ENDPOINT2;
          break;
        }

      bool onOff;
      if (emberAfReadServerAttribute (endpoint, ZCL_ON_OFF_CLUSTER_ID,
      ZCL_ON_OFF_ATTRIBUTE_ID, (uint8_t*) &onOff, sizeof(onOff))
        ...
        emberEventControlSetDelayMS(commissioningLedEventControl, 2000);
    }
}
